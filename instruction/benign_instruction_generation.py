...
def prompt_construction(params):
    ...
    prompt_file = os.path.join(cur_dir, "seed_questions", "benign_instruction_prompt.txt")
...
def benign_instruction_generation(all_queries):
    """
    item : {
        "pattern": [
            "Search.web_search",
    ...
    output_file = os.path.join(cur_dir, "generated_instructions","benign_instructions_{}_patch_round_{}.json".format(model_name, 1))
    ...
    for idx, query in enumerate(tqdm(all_queries)):
        ...
        if idx % 5 == 0:
            with open(output_file, "w", encoding="utf-8") as f:
                json.dump(benign_instructions, f, indent=4)
...
if __name__ == "__main__":
    ...
    benign_instruction_generation(all_queris)